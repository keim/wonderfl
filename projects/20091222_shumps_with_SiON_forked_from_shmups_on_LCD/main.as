// forked from keim_at_Si's shmups on LCD
...
package {
    ...
    public class main extends Sprite {
        function main() {
            ...
            // lcd render
            _lcd = new LCDRender(2, 0x080820, 0x203040, 0xc0c0c0, 0.4);
            ...
            _actorManager[ENEMY].pat("583c563f3f563c58", 7, 6).pat("2b7e353c3c357e2b", 14, 20);
            _actorManager[ENEMY].pat("2b7e353c3c357e2b", 21, 20).pat("01", 1);
            ...
            new PushButton(this, 190, 438, "tweet score", function(e:Event):void {
                var url: String = "http://twitter.com/home/?status=";
                url += escapeMultiByte("[STG with SiON] Score:") + String(score);
                url += " http://bit.ly/7A3PEZ"
...
var _lcd:LCDRender;
var _key:KeyMapper;
var _resourceManager:ResourceManager = new ResourceManager();
...
class LCDRender extends Bitmap {
    ...
    static public function hex2bmp(hex:String, height:int=8, scale:int=1) : BitmapData {
        ...
        for (i=0, rect.x=0; i<hex.length; i+=2, rect.x+=scale)
            for (rect.y=0, pat=parseInt(hex.substr(i, 2), 16); pat!=0; rect.y+=scale, pat>>=1) {
                bmp.fillRect(rect, (pat&1)*0xffffffff);
            }
        return bmp;
    }
    ...
    public function render() : void {
        ...
        _screen.applyFilter(_screen, _screen.rect, _screen.rect.topLeft, _glow);//_shadow);
...
class ResourceManager {
    ...
    private var _bgmData:SiONData, _baseData:SiONData;
    function ResourceManager() {
        _driver.setSamplerData(1, _driver.render("t160;#A=@v96q0s32l32o7e-crb-;A"));
        _driver.setSamplerData(2, _driver.render("t160;#A=%1@1@v128p2q0s32l32o5c<fcr<gb-f;k4A;k-4A"));
        _driver.setSamplerData(3, _driver.render("t160;#A=@v128p2@al2i1@dt760q0s32l32o5cg<cfb-e-b-<e-;k4A;k-4A"));
        _bgmData = _driver.compile("t160;#EFFECT1{delay281,30,1};%10o0q8@v128v12$c1");
        _bgmData.setSamplerData(0, _driver.render("t160;#EFFECT0{ws};%5@4q0s36,-128@v128v28o3l4[g]4;%2@1q0s36v12kt-12l4rcrc8.)8c16(8;%2@3q0o5v12s44l16[aar8]4;%2@3q0o5v8s28l16[rra8]4"));
        _baseData = _driver.compile("%5@1q4@f64,2v16o2l16$ff<f8>");
        _driver.autoStop = true;
        _driver.noteOnExceptionMode = SiONDriver.NEM_OVERWRITE;
        _driver.addEventListener(SiONEvent.STREAM_START, _onStreamStart);
    }
    
    public function startBGM() : void {
        _driver.play(_bgmData);
    }
    ...
    public function playSound(index:int) : void {
        var track:SiMMLTrack = _driver.playSound(index, 4, 0, 1);
        track.velocity = 96;
        track.channel.setStreamSend(1, 0.3);
    }
    
    private function _onStreamStart(e:SiONEvent) : void {
        _driver.fadeIn(3);
        _driver.sequenceOn(_baseData);
    }
}
...
class SceneManager {
    ...
    private var _groupScript:Array = [
        ["&c2&p3p6v12[5f{&l1vd24i20vw10f6w10vd-20}w3]ko",
         "&c2&p3p90v-12[5f{&l1vd24i20vw10f6w10vd-20}w3]ko", 
         "&p1&l15v0,12i8v0,1w8ha-50f8hs20[5w8f8]w5ha50f8hs-20[5w8f8]w5a0,1",
         "[40ha90f100{w1b4,120f10{&p1}ko}ha-90f100w30]"],
        ["&c2&p3p12v24[4f{&l2v0,20i10vw10ht[6f-10{i30hv180vd20}w1hs0]w30hti60vd40}w2]ko",
         "&c2&p3p84v-24[4f{&l2v0,20i10vw10ht[6f-10{i30hv180vd20}w1hs0]w30hti60vd40}w2]ko", 
         "&p1&l15v0,12i8v0,1w12b2,45f10{&p1}w24ht-10f10w48a0,1",
         "[20b4,160htf8{&p0}w4b1ha135[5f15{&p1i10vw10vd20}w1]w16ha215[5f15w1]w12]"],
        ["&c2&p3p6v12[5f{&l1b2,0,2vd24i20vw10f8w10vd-20}w3]ko",
         "&c2&p3p90v-12[5f{&l1b2,0,2vd24i20vw10f8w10vd-20}w3]ko", 
         "&p1&l15v0,12i8v0,1w16b4,90f10{&p1}w32f10w48a0,1",
         "@{[20haf20{&p1i15vw20hab12,330f5w10ha15b12,330f10ko}w64]}[30ha90f50{w1vd-4w1hv[4f-15{&p1}w2]ko}w24ha-90f50w24]"],
        ["&c2&p3p4,16[3f{&l3hr90[2v20,1w6f8w12v-20,1w6f8w12]}w3]ko",
         "&c2&p3p92,16[3f{&l3hr90[2v-20,1w6f8w12v20,1w6f8w12]}w3]ko", 
         "&p1&l15v0,12i8v0,1w16[2b3,60f1{&p1hvi40vd12}w32]a0,1",
         "b4,120[40ht40f6{&p1}hs-10bv2[4w2f]w18ht-40f6hs10bv2[4w2f]w18]"],
        ["&c2&p3p16v10[3f{&l2v0,48i10vw10a0,-1hr90b3,240[f4w24]}w5]fko",
         "&c2&p3p80v-10[3f{&l2v0,48i10vw10a0,-1hr90b3,240[f4w24]}w5]ko",
         "&p1&l15v0,12i8v0,1w16b2,10ht4f6hs-1bv1[7fw2]ht-4f6hs1bv1[7fw2]w24a0,1",
         "ha[40b1f{vb7,120f8{&p1}hsw20b6,100f16{&p0}ko}[8w1b2,60f-10{&p1[60ad1w2]}]w24]"],
        ["&c2&p3p80[2f{&l4i4v0,20w4bv2f6[2v-20,0w4v0,-16fw4v20,0w4v0,24fw4]}w8]ko",
         "&c2&p3p16[2f{&l4i4v0,20w4bv2f6[2v20,0w4v0,-16fw4v-20,0w4v0,24fw4]}w8]ko",
         "&p1&l15v0,12i8v0,1w12ha-48f6{&p1}hs16bv1[5w2f]w6ha40f6hs-16bv1[5w2f]w24a0,1",
         "@{b3,240hs10[320f20{&p1hv90w5f10}w4]}b3,240hs-10[320f20{&p1hv90w5f10}w4]"]
    ];

    function SceneManager() {
        _stageScript  = "&c2&p3&l0,1p48w64f$0w32f$1w32f$0w16f$1w16f$2w32p24f$2p72f$2w64";
        _stageScript += "f$0w12p64f$2w24f$1w12p32f$2w64p48f$3ko";
        for (var i:int=0; i<_groupScript.length; i++) {
            _groupScript[i][3] = "&p2&l64v0,20i8v0,1w8@{v10[10i64v-10w64v10w64]a0,1}"+_groupScript[i][3];
        }
    }
    ...
    public function changeScene(id:int) : void {
        ...
        case 2: 
            _stage.kill();
            _resourceManager.stopBGM();
            break;
        }
    }
    
    private function _start() : void {
        if (phase == _groupScript.length) { 
            _player.clearShot();
            _resourceManager.stopBGM();
            score += _player.life * 2000;
            changeScene(3); 
            return;
       }
    ...
    public function update() : Boolean {
        ...
        switch (id) {
        case 0:
            _lcd.print(3, 32, "-STG WITH SION-"); _lcd.print(6, 64, "PRESS SHOT KEY");
...
class Actor {
    ...
    public function update(eval:ActorManager) : Actor {
        ...
        if (eval && life>0 && (life-=eval.hitEval(this, _manager._size2[patNum]))<=0) {
            score += bonus * ((_player.y<48)?2:1);
            _actorManager[EXPLOSION].alloc().init(x, y, vx, vy+1).vel(0,0,20);
            _resourceManager.playSound((bonus<100)?1:(bonus<600)?2:3);
